...
$segundaConta = new ContaCorrente($anaJulia);
...
$terceiraConta = new ContaCorrente(new Titular(new Cpf('951.147.852-65'),'Eli Silva', $enderecoGenerico));

$helenaClara = new Titular(new Cpf('258.147.483-20'), 'Helena Clara', $enderecoGenerico);
$terceiraConta = new ContaCorrente($helenaClara);//note que eu estou criando uma nova instância de Conta, mas com um aváriavel já utilizada. Isso faz com que a instância acima fique sem referência na memória. Por padrão p php vai excluir a instância acima. Com isso o método  destrutor vai ser chamado. No nosso caso, embora o código mostre 4 contas criadas, o contador vai mostrar 3, já que a conta acima foi excluída memória.

echo Conta::retornarNumeroDeContas() . PHP_EOL;

$umFuncionario = new Desenvolvedor('Maria Eduarda', new Cpf('123.456.789-10'), 1200);
